...
PROGRAM DESCRIPTION    : circular queue using array with variable queue data linked using SLL in queue node,
...
{
	SUCCESS, NO_ERROR = 0, FAILURE, ERR_NULL_PTR, ERR_INVALID_DATA, ERR_FORMAT_INVALID , ERR_QUEUE_FULL, ERR_QUEUE_EMPTY, ERR_ENQUEUE_PROC, ERR_DEQUEUE_PROC, ERR_QUEUE_INSERT_FRONT_PROC, ERR_QUEUE_DELETE_REAR_PROC,
...
/* queue's node data has Employee's record */
#define EMP_MSG                                1
/* queue's node data has num data */
#define NUM_MSG                                2
...
unsigned int Reset_Circular_Queue(const unsigned int reset_mode, circular_queue_metahead_t *const circular_queue_metahead_ptr)
{
	unsigned int cur_arr_index;
  	...
  	if(circular_queue_metahead_ptr == NULL_DATA_PTR)
	{
...
unsigned int EnQueue_Proc(circular_queue_metahead_t *const circular_queue_metahead_ptr, const unsigned int data_len, void *insert_data_ptr)
{
	...
	if(insert_data_ptr == NULL_DATA_PTR)
	{
		system_status_flag = ERR_NULL_PTR;
...
unsigned int Insert_EmpData(const unsigned int insert_mode, circular_queue_metahead_t *const circular_queue_metahead_ptr,  queue_data_t *insert_emp_ptr)
{
	...
	if(insert_emp_ptr == NULL)
	{
		...
		printf("\n Enter Emp Name : ");
		if((Get_Input_Alpha_Char_Str(name, MAX_NAME_CHARS)) != SUCCESS)
		{
...
unsigned int Insert_Queue_Data(const unsigned int insert_mode, circular_queue_metahead_t *const circular_queue_metahead_ptr )
{
	...
	printf("\n Enter : ");
	//scanf("%u", &msgtype);
	if((Get_Validate_Input_Number(&msgtype, input_str, STR_MAX_CHARS, EMP_MSG, NUM_MSG)) != SUCCESS)
	{
...
unsigned int Insert_NumData(const unsigned int mode, circular_queue_metahead_t *const circular_queue_metahead_ptr, queue_data_t *enqueue_num_ptr)
{
	...
	if(enqueue_num_ptr == NULL)
	{
	    printf("\n Enter a num : ");
		// scanf("%d", &num);
		if((Get_Validate_Input_Number(&num, input_str, STR_MAX_CHARS, MSG_ID_NUM_MIN_VAL, MSG_ID_NUM_MAX_VAL)) != SUCCESS)
		{
			 printf("\n ERR[13.0]: Invalid number as message");
			 return ERR_FORMAT_INVALID;
		}
...
unsigned int DeQueue_Proc(circular_queue_metahead_t *const circular_queue_metahead_ptr, void **const dequeue_data_ptr)
{
	...
	if(dequeue_data_ptr == NULL_DATA_PTR)
	{
		system_status_flag = ERR_NULL_PTR;
...
unsigned int Queue_Delete_At_Rear(circular_queue_metahead_t *const circular_queue_metahead_ptr, void **const deleted_data_ptr)
{
	...
	if(deleted_data_ptr == NULL_DATA_PTR)
	{
		system_status_flag = ERR_NULL_PTR;
...
unsigned int Queue_Disp_Info(circular_queue_metahead_t *const circular_queue_metahead_ptr, void **retrieve_data)
{
	...
	printf("\n enter disp choice : ");
	//scanf("%d", &ch);
	if((Get_Validate_Input_Number(&ch, input_str, STR_MAX_CHARS, RETRIEVE_QUEUE_FRONT_DATA_OPER, DISP_QUEUE_METADATA_OPER)) != SUCCESS)
    {
		printf("\n ERR[19.0]: Invalid queue display");
		return ERR_FORMAT_INVALID;
	}
	switch(ch)
	{
...
int main()
{
    ...
    while(1)
    {
        ...
        //scanf("%d",&ch);
		if((Get_Validate_Input_Number(&ch, input_str, STR_MAX_CHARS, ENQUEUE_OPER, EXIT_OPER)) != SUCCESS)
        {
		    printf("\n ERR[19.0]: Invalid queue operation ");
		    continue;
	    }
        switch(ch)
        {
